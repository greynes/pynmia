...
#    entropy = -np.sum(p_xpy[p_xpy>0] * np.log10(p_xpy[p_xpy>0]))
    ...
    autocorrelation = np.mean(autocorrelation)
...
#    max
    ...
    # Coefficients
     ...
     if verbose is 1:
         ...
         print('LGRE', "%.4f" % lglre)
         print('HGRE', "%.4f" % hglre)
     varnames = ['GLRM_SRE', 'GLRM_LRE', 'GLRM_LGRE', 'GLRM_HGRE']
